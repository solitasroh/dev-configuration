...
class LMHInfoData implements A2700Data {
  ...
  setSerialNumber(serialNumber: number): void {
    this.serialNumber = LMHInfoData.fillZero(6, serialNumber.toString());
  }
  ...
  static getAppVersion(version: number): string {
    ...
    return `${main}.${sub}.${LMHInfoData.fillZero(3, fixed.toString())}`;
  }
