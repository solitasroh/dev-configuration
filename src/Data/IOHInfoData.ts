...
class IOHInfoData implements A2700Data {
  ...
  setSerialNumber(serialNumber: number): void {
    this.serialNumber = IOHInfoData.fillZero(6, serialNumber.toString());
  }
  ...
  static getAppVersion(version: number): string {
    ...
    return `${main}.${sub}.${IOHInfoData.fillZero(3, fixed.toString())}`;
  }
