...
export default class A2750LDInformationReg extends RegisterBase {
    setter = (data: A2700Data): void => {
      console.log(`empty setter ${data.type}`);
    }
  
    getter = (): Observable<A2700Data>  => ModbusService.read(61524, 10).pipe(
        map((data) => {
          ...
          result.applicationVersion = LDInformation.getAppVersion(applicationVersion);
          result.bootloaderVersion = LDInformation.getAppVersion(bootloaderVersion);;
